...
class chakra(Env):
    ...
    def _step(self, action):
        ...
        self.state += action
        rew = (-1)*np.linalg.norm(self.state)
        if self.state[0] > 1 or self.state[0] < -1 or self.state[1] > 1 or self.state[1] < -1 :
            self.state = self.reset()
            # rew = -300 ## EXPERIMENTAL, NOT IN PS
        return self.state, rew,done,{}   # Return the next state and the reward, along with 2 additional quantities : False, {}
